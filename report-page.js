...
(async function () {
  ...
  function render(r, profileBlob) {
    ...
    parts.push(`
      <div class="mri-method">
        ...
        ${renderInstrumentNote(r.instrument)}
      </div>`);
  ...
  // Says so when the findings above pool decisions from more than one version
  // of the sandbox. Silence here is a claim too: it says every decision came
  // from the same instrument, which is only worth saying if it is checked.
  function renderInstrumentNote(inst) {
    if (!inst || !inst.mixed) return "";
    const reasons = [];
    if (inst.versions.length > 1) reasons.push("the scenario set changed between your sessions");
    if (inst.edited && inst.edited.length) {
      reasons.push(`${inst.edited.length} scenario${inst.edited.length === 1 ? " was" : "s were"} reworded after you first answered ${inst.edited.length === 1 ? "it" : "them"}`);
    }
    if (inst.unstamped) reasons.push(`${inst.unstamped} decision${inst.unstamped === 1 ? " predates" : "s predate"} version tracking`);
    if (!reasons.length) return "";
    return `<p><strong>A limit on the above.</strong> Your decisions did not all come from the same version of the sandbox: ${reasons.join(", and ")}. Comparisons across that boundary are weaker than the numbers make them look.</p>`;
  }
