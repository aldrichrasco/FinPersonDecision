...
- `ml/` — predictive models on product data (Pro conversion likelihood, streak drop-off) AND the archetype-matcher rebuild (empirical clustering vs. `fbm.js`'s hand-defined archetypes, logistic-regression/random-forest classifier, exported model artifact): real feature engineering against the DB schema, evaluated properly (PR-AUC / confusion matrices, not raw accuracy), with an honest data-quality gate that currently falls back to synthetic, documented-relationship datasets since real product volume doesn't exist yet — see `ML_ENGINEERING_NOTES.md` §§2-3 for the full write-ups. `pip install -r requirements-ml.txt` first
- `.github/workflows/ml_retrain.yml` / `ml/drift.py` — scheduled (+ on-`ml/**`-push, + on-demand) retrain-validate-version pipeline for all three models above, with a Population Stability Index drift check against a git-tracked baseline and a real validation gate that fails the CI job on a bad model — see `ML_ENGINEERING_NOTES.md` §4
- `crypto.py` / `crypto-impulse.html` — Crypto Impulse Check: a real historical BTC/ETH volatility event (CoinGecko's free, no-key public API, 5-minute server-side cache), shown without its outcome; you decide, then the real next-14-days price path is revealed. Deliberately doesn't verdict the choice as right/wrong — one historical instance doesn't validate a strategy — only connects it to the person's own saved impulse-regulation/risk-disposition axis scores when a profile exists, same translation-layer tone as the rest of the app
